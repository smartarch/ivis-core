...
router.postAsync('/cloud/:serviceId/preset-types', passport.loggedIn, async (req, res) => {
    ...
    return res.json(await cloud_services.listTypesDTAjax(req.context, castToInteger(req.params.serviceId)));
});
...
router.postAsync('/cloud/:serviceId/preset', passport.loggedIn, passport.csrfProtection, async (req, res) => {
    ...
    preset.service = castToInteger(req.params.serviceId);
    return res.json(await presets.create(req.context, preset));
});
