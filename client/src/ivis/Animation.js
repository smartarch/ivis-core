...
import {controlLayouts} from "../lib/media-controls";
...
class Animation extends Component {
    ...
    render() {
        const ControlLayout = controlLayouts[this.props.config.controlGroup];
        const Context = this.props.config.type === 'client' ? ClientAnimation : ServerAnimation;

        return (
            <Context config={this.props.config}>
                {this.props.config.controlsPlacement === 'before' && <ControlLayout animationConf={this.props.config}/>}
                {this.props.children}
                {this.props.config.controlsPlacement === 'after' && <ControlLayout animationConf={this.props.config}/>}
            </Context>
        );
    }
}

//TODO: prettier & css based
